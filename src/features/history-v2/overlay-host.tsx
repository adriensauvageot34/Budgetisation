...
export function HistoryOverlayHost({
  ...
  return (
    <OverlayFrame
      ...
      subtitle={`Niveau ${stack.length} sur 6`}
      kind="day_drawer"
...
function JournalTimeline({ model, onPush }: { readonly model: JournalDayReadModel; readonly onPush: (target: HistoryOverlayTarget) => void }) {
  return <div className={styles.journalSections}>
    <DisplayState node={model.contexts}>{(collection) => <CollectionState collection={collection}>{(items) => <section><h3>Contextes du jour</h3><div className={styles.participants}>{items.map((item) => <span key={`${item.personId}-${item.contextTypeKey}`}>{item.label}</span>)}</div></section>}</CollectionState>}</DisplayState>
    <DisplayState node={model.activeContinuousEvents}>{(collection) => <CollectionState collection={collection}>{(items) => <section><h3>Événements continus</h3>{items.map((item) => <article key={item.calendarItemId}><strong>{item.title}</strong><span>{item.startDate} → {item.endDate} · jour {dayOffset(model.date, item.startDate)}/{dayOffset(item.endDate, item.startDate)}</span></article>)}</section>}</CollectionState>}</DisplayState>
    <DisplayState node={model.timedTimeline}>{(collection) => <CollectionState collection={collection}>{(items) => <section><h3>Chronologie</h3>{items.map((item) => <article key={item.calendarItemId}><time>{item.startTime}</time><div><strong>{item.title}</strong>{item.placeLabel ? <span>{item.placeLabel}</span> : null}{item.moment ? <><span>Coût causal : <DisplayState node={item.moment.causalCost}>{(metric) => <MoneyMetric metric={metric} />}</DisplayState></span><span>Dépensé pendant : <DisplayState node={item.moment.spentDuring}>{(metric) => <MoneyMetric metric={metric} />}</DisplayState></span><button type="button" className="button-ghost" onClick={() => onPush({ kind: "moment", momentId: item.moment!.momentId })}>Ouvrir le moment</button></> : null}</div></article>)}</section>}</CollectionState>}</DisplayState>
    <DisplayState node={model.untimedEvents}>{(collection) => <CollectionState collection={collection}>{(items) => <section><h3>Sans horaire précisé</h3>{items.map((item) => <article key={item.calendarItemId}><div><strong>{item.title}</strong>{item.placeLabel ? <span>{item.placeLabel}</span> : null}{item.moment ? <><span>Coût causal : <DisplayState node={item.moment.causalCost}>{(metric) => <MoneyMetric metric={metric} />}</DisplayState></span><span>Dépensé pendant : <DisplayState node={item.moment.spentDuring}>{(metric) => <MoneyMetric metric={metric} />}</DisplayState></span><button type="button" className="button-ghost" onClick={() => onPush({ kind: "moment", momentId: item.moment!.momentId })}>Ouvrir le moment</button></> : null}</div></article>)}</section>}</CollectionState>}</DisplayState>
    <details><summary>Autres mouvements</summary><div className={styles.otherMovements}><ExpenseCollection title="Autres dépenses" node={model.otherMovements.otherExpenses} /><MoneyCollection title="Remboursements et ajustements" node={model.otherMovements.refundsAndAdjustments} /><MoneyCollection title="Entrées" node={model.otherMovements.inflows} /><MoneyCollection title="Mouvements techniques" node={model.otherMovements.technicalMovements} /></div></details>
  </div>;
}
...
function ExpenseCollection({ title, node }: { readonly title: string; readonly node: JournalDayReadModel["otherMovements"]["otherExpenses"] }) {
  return <DisplayState node={node}>{(collection) => <CollectionState collection={collection}>{(items) => <section><h4>{title}</h4>{items.map((item) => <div key={item.expenseEventId}><span>{item.label}</span><strong>{formatMoney(item.amount)}</strong></div>)}</section>}</CollectionState>}</DisplayState>;
}

function MoneyCollection<T extends { readonly movementId: string; readonly label: string; readonly amount: import("@/core/money").Money }>({ title, node }: { readonly title: string; readonly node: import("@/core/history-v2").DisplayNode<import("@/core/history-v2").CollectionValue<T>> }) {
  return <DisplayState node={node}>{(collection) => <CollectionState collection={collection}>{(items) => <section><h4>{title}</h4>{items.map((item) => <div key={item.movementId}><span>{item.label}</span><strong>{formatMoney(item.amount)}</strong></div>)}</section>}</CollectionState>}</DisplayState>;
}
...
function ActivityPanel({ month, activityTypeKey, onPush }: { readonly month: YearMonth; readonly activityTypeKey: string; readonly onPush: (target: HistoryOverlayTarget) => void }) {
  ...
  return <StateBoundary state={state}>{(model: ActivityDetailReadModel) => <div><header className={styles.detailHeader}><h3>{model.activity.label}</h3>{model.activity.costKind === "NONE" ? null : <MoneyMetric metric={model.activity.cost} />}</header><DisplayState node={model.frequencyTicket}>{(diagnostic) => diagnostic.availability === "KNOWN" ? <p>Diagnostic fréquence × ticket : {diagnostic.dominantFactor}</p> : null}</DisplayState><DisplayState node={model.occurrences}>{(collection) => <CollectionState collection={collection}>{(items) => <section><h4>Occurrences</h4>{items.map((item) => <div className={styles.activityOccurrence} key={item.occurrenceId}><button type="button" className="button-ghost" onClick={() => onPush({ kind: "journal", date: item.effectiveDate })}><span>{item.effectiveDate}{item.effectiveTime ? ` · ${item.effectiveTime}` : ""}</span><span>Ouvrir le Journal</span></button><div className={styles.relatedTargets}>{item.momentIds.map((momentId) => <button type="button" className="button-ghost" key={momentId} onClick={() => onPush({ kind: "moment", momentId })}>Moment lié</button>)}{item.placeIds.map((placeId) => <button type="button" className="button-ghost" key={placeId} onClick={() => onPush({ kind: "place", placeId })}>Lieu lié</button>)}{item.categoryIds.map((categoryId) => <button type="button" className="button-ghost" key={categoryId} onClick={() => onPush({ kind: "category", categoryId })}>Catégorie liée</button>)}</div></div>)}</section>}</CollectionState>}</DisplayState><ExpenseCollection title="Dépenses causales" node={model.causalExpenses} /><ExpenseCollection title="Dépenses associées" node={model.associatedExpenses} /></div>}</StateBoundary>;
}
...
function MomentPanel({ month, momentId }: { readonly month: YearMonth; readonly momentId: string }) {
  ...
  return <StateBoundary state={state}>{(model: MomentDetailReadModel) => <div><header className={styles.detailHeader}><h3>{model.moment.title}</h3><span>{model.moment.startDate}{model.moment.endDate ? ` → ${model.moment.endDate}` : ""}</span></header><div className={styles.dualMetrics}><div><span>Coût causal</span><DisplayState node={model.causalCost}>{(metric) => <MoneyMetric metric={metric} />}</DisplayState></div><div><span>Dépensé pendant</span><DisplayState node={model.spentDuring}>{(metric) => <MoneyMetric metric={metric} />}</DisplayState></div></div><ExpenseCollection title="Dépenses causales" node={model.causalExpenses} /><ExpenseCollection title="Dépenses dans la fenêtre" node={model.spentDuringExpenses} /></div>}</StateBoundary>;
}
