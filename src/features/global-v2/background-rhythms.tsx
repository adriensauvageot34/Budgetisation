...
export function BackgroundRhythms({ runtime, onMethod }: { readonly runtime: GlobalV2VisitRuntime; readonly onMethod: () => void }) {
  ...
  return <section ref={nearViewport.ref} className={styles.backgroundRhythms} aria-labelledby="background-rhythms-title">
    ...
    {model === undefined ? result.state.status === "ERROR" ? <div className={styles.backgroundError} role="alert"><p>Nos rythmes de fond n’ont pas pu être chargés. La Timeline reste disponible.</p><button type="button" onClick={result.retry}>Réessayer</button></div> : <BackgroundRhythmsSkeleton /> : <div className={styles.rhythmCards}><FoodRhythmRiver food={model.food} /><CarMobilityRoute car={model.carMobility} runtime={runtime} onTimelineDestination={focusTimelineDestination} /></div>}
  </section>;
}
