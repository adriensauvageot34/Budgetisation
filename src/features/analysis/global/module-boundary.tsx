...
export function AnalysisGlobalModuleBoundary<T>({
  route,
  module,
  state,
  children,
  ...
  readonly module: "baseline" | "typical" | "evolution" | "habits" | "profiles" | "universe";
  ...
  return <>{children(data)}{state.status === "success" && state.refreshing ? <RefreshIndicator announce /> : state.status === "error" ? <RefreshIndicator failed announce /> : null}</>;
}
