...
const currentRegime = buildGlobalCurrentRegime({ points: months.map((month) => ({ month, status: "KNOWN", value: "1", complete: true, eligible: true, corpus: "CERTIFIED_HISTORY", dependencyRefs: [`fixture:${month}`] })), certifiedThroughMonth: "2025-12", regime: { kind: "DURABLE_CHANGE", status: "CONFIRMED_ONGOING", start: "2025-01", evidenceRefs: ["fixture:canonical-change"] } });
const regimeAuthority = { status: "KNOWN", capabilityState: "AVAILABLE", reasonCodes: [], personId, sourceActivityId: "travail_site", sourceSignalId: `m4:person:${personId}:activity:travail_site:frequency`, transformationId: "fixture-work-regime", validFrom: "2025-01", validThrough: "2025-12-31", result: currentRegime };
const run = (data = tables, extra = {}) => resolveGlobalM5PersonAuthority({ repository: new CanonicalRepository(clientFor(data), context), scope, regimeAuthority, ...extra });
const result = await run();
let count = 0;
const check = (fn) => { fn(); count++; };
check(() => assert.equal(result.evaluationStatus, "READY_FOR_PRODUCT"));
check(() => assert.equal(result.providerStatus.restaurant, "CANONICAL_PARTICIPATION_CONNECTED"));
check(() => assert.equal(result.resolution.certifiedUnitIds.length, 365));
check(() => assert.ok(result.analysis.outcomes.some((row) => row.value === 1 && row.status === "KNOWN")));
check(() => assert.deepEqual(result.relationships, []));
check(() => assert.deepEqual(result.executedRelationshipStages, []));
check(() => assert.equal(Object.hasOwn(result, "current"), false));
...
check(() => assert.ok(noParticipation.analysis.outcomes.every((row) => row.status === "UNKNOWN")));
check(() => assert.notEqual(noParticipation.inputHash, result.inputHash));
const readsBeforeGate = reads.length;
const noRegime = await resolveGlobalM5PersonAuthority({ repository: new CanonicalRepository(clientFor(tables), context), scope });
check(() => assert.equal(noRegime.evaluationStatus, "AUTHORITY_GATED"));
check(() => assert.equal(noRegime.providerStatus.regime, "AUTHORITY_GATED"));
check(() => assert.deepEqual(noRegime.reasonCodes, ["AUTHORITY_GATED_CURRENT_REGIME"]));
check(() => assert.deepEqual(noRegime.executedRelationshipStages, []));
check(() => assert.equal(Object.hasOwn(noRegime, "current"), false));
check(() => assert.equal(Object.hasOwn(noRegime, "fdr"), false));
check(() => assert.equal(reads.length, readsBeforeGate));
const conflictRegime = await resolveGlobalM5PersonAuthority({ repository: new CanonicalRepository(clientFor(tables), context), scope, regimeAuthority: { status: "CONFLICT", capabilityState: "GATED", reasonCodes: ["CONFLICTING_CURRENT_REGIME_AUTHORITIES"], personId } });
check(() => assert.equal(conflictRegime.evaluationStatus, "AUTHORITY_GATED"));
check(() => assert.deepEqual(conflictRegime.executedRelationshipStages, []));
const secondPersonId = uuid(999);
const twoPersonContext = { ...context, personIds: [personId, secondPersonId], persons: [...context.persons, { personId: secondPersonId, householdId }] };
const secondPersonScope = { ...scope, subject: { kind: "person", personId: secondPersonId } };
const secondPersonGate = await resolveGlobalM5PersonAuthority({ repository: new CanonicalRepository(clientFor(tables), twoPersonContext), scope: secondPersonScope, regimeAuthority: { status: "UNKNOWN", capabilityState: "GATED", reasonCodes: ["AUTHORITY_GATED_CURRENT_REGIME"], personId: secondPersonId } });
check(() => assert.equal(secondPersonGate.evaluationStatus, "AUTHORITY_GATED"));
check(() => assert.deepEqual(secondPersonGate.executedRelationshipStages, []));
...
check(() => assert.ok(partial.analysis.outcomes.every((row) => row.status !== "KNOWN")));
...
check(() => assert.equal(result.exceptionPolicy, "NOT_USED_V1"));
check(() => assert.equal(result.seasonPolicy, "NOT_REQUIRED"));
...
check(() => assert.deepEqual(missingDay.executedRelationshipStages, []));
const unknownRestaurantParticipation = await run({ ...tables, life_event_participations: tables.life_event_participations.map((row) => ({ ...row, participation_status: "Inconnue" })) });
check(() => assert.ok(unknownRestaurantParticipation.analysis.outcomes.every((row) => row.status === "UNKNOWN")));
check(() => assert.equal(unknownRestaurantParticipation.insights.length, 0));
const workLabels = await run({ ...tables, life_event_types: [{ ...tables.life_event_types[0], type_key: "travail_site" }] });
check(() => assert.equal(workLabels.providerStatus.workContext, "CANONICAL_PARTICIPATION_CONNECTED"));
check(() => assert.ok(workLabels.analysis.days.some((row) => row.context === "ONSITE")));
