...
const outputByModule = {
  ...
  GEO_MOBILITY: {
    ...
    personalMobilitySummaries: [
      { entityRef: "personal-mobility:bridge-work", personId: personA, contextKind: "WORK_COMMUTE", scope: "PERSONAL", couplePresenceFilter: { state: "ANY" }, legCount: 8, distinctDayCount: 4, eventCount: 0, distanceKm: "120", durationSeconds: { status: "KNOWN", value: "7200", knownLegCount: 8, eligibleLegCount: 8 }, estimatedFuelLiters: "9", estimatedFuelCost: "18", mobilityCostMetric: { metricId: "mobility_usage_estimated_fuel_cost", methodVersion: "mobility_usage_estimated_fuel_cost@v1", provenance: "estimated", monetaryBasis: "estimated_cost" }, grossMobilityUsage: { status: "READY" }, incrementalMobilityCost: { status: "UNAVAILABLE", reasonCode: "COUNTERFACTUAL_MODEL_UNAVAILABLE" }, firstObservedDate: "2026-06-01", lastObservedDate: "2026-06-04", support: { status: "SUFFICIENT", observedLegCount: 8, distinctDayCount: 4, minimumDistinctDays: 2, policyRef: "global-m7-personal-mobility-distinct-day-support@v1" }, knowledgeState: "KNOWN", temporalQualityDistribution: { EXACT: 8, APPROXIMATE: 0, DATE_ONLY: 0, PROXY: 0, UNKNOWN: 0 }, sourceConfidenceDistribution: { HIGH: 8, MEDIUM: 0, LOW: 0, UNKNOWN: 0 }, detailRef: { resource: "analysis_global_place_mobility_detail", entityRef: "personal-mobility:bridge-work", role: "PRIMARY" }, evidenceRefs: ["owner:mobility-work"], inputHash: "9".repeat(64), rawLegs: ["must-not-copy-raw-leg"], contextLinks: ["must-not-copy-context-link"] },
      { entityRef: "personal-mobility:bridge-friend", personId: personB, contextKind: "FRIEND_VISIT", scope: "PERSONAL", couplePresenceFilter: { state: "OTHER_ELSEWHERE_CONFIRMED", otherPersonId: personA }, legCount: 2, distinctDayCount: 2, eventCount: 1, distanceKm: "20", durationSeconds: { status: "KNOWN", value: "1800", knownLegCount: 2, eligibleLegCount: 2 }, estimatedFuelLiters: "1.5", estimatedFuelCost: "3", mobilityCostMetric: { metricId: "mobility_usage_estimated_fuel_cost", methodVersion: "mobility_usage_estimated_fuel_cost@v1", provenance: "estimated", monetaryBasis: "estimated_cost" }, grossMobilityUsage: { status: "READY" }, incrementalMobilityCost: { status: "UNAVAILABLE", reasonCode: "COUNTERFACTUAL_MODEL_UNAVAILABLE" }, firstObservedDate: "2026-06-10", lastObservedDate: "2026-06-11", support: { status: "SUFFICIENT", observedLegCount: 2, distinctDayCount: 2, minimumDistinctDays: 2, policyRef: "global-m7-personal-mobility-distinct-day-support@v1" }, knowledgeState: "KNOWN", temporalQualityDistribution: { EXACT: 2, APPROXIMATE: 0, DATE_ONLY: 0, PROXY: 0, UNKNOWN: 0 }, sourceConfidenceDistribution: { HIGH: 2, MEDIUM: 0, LOW: 0, UNKNOWN: 0 }, detailRef: { resource: "analysis_global_place_mobility_detail", entityRef: "personal-mobility:bridge-friend", role: "PRIMARY" }, evidenceRefs: ["owner:mobility-friend"], inputHash: "8".repeat(64) },
    ],
...
const bridgeWorkMobilityBlock = personADetail.blocks.find(({ semanticKey }) => semanticKey === "mobility:work-commute");
const bridgeFriendMobilityBlock = personBDetail.blocks.find(({ semanticKey }) => semanticKey === "mobility:friend-visit:without-household-partner-confirmed");
...
check(() => assert.deepEqual(bridgeWorkMobilityBlock.detailRefs, [{ resource: "analysis_global_place_mobility_detail", entityRef: "personal-mobility:bridge-work", role: "PRIMARY" }]));
check(() => assert.deepEqual(bridgeFriendMobilityBlock.detailRefs, [{ resource: "analysis_global_place_mobility_detail", entityRef: "personal-mobility:bridge-friend", role: "PRIMARY" }]));
check(() => assert.deepEqual(bridgeWorkMobilityBlock.surfaceMetrics, []));
...
const ownerWorkMobilityDetail = queryDetail("analysis_global_place_mobility_detail", "personal-mobility:bridge-work");
...
check(() => assert.deepEqual(ownerWorkMobilityDetail.metrics.map(({ metricId, displayValue }) => [metricId, displayValue]), [["distance-km", "120 km"], ["distinct-day-count", "4"], ["estimated-fuel-cost", "18 €"], ["estimated-fuel-liters", "9 L"], ["event-count", "0"], ["leg-count", "8"]]));
check(() => assert.equal(ownerWorkMobilityDetail.metrics.some(({ labelKey }) => /visite/u.test(labelKey)), false));
...
check(() => assert.equal(personaDetailSnapshots.every(({ payload }) => !/must-not-copy-raw-leg|must-not-copy-context-link/u.test(JSON.stringify(payload.personaDetailIndex))), true));
...
check(() => assert.match(orchestratorSource, /const \[m2, m6, m7Place, m7PersonalMobility, productObservations\][\s\S]*resolveGlobalM7PersonalMobilityAuthority[\s\S]*resolveGlobalPersonaProductObservations[\s\S]*const baseM3Evaluation[\s\S]*const personRegimeAuthorities[\s\S]*const m5 = await Promise\.all[\s\S]*const m5Product = buildGlobalM5Pr03Product/u));
