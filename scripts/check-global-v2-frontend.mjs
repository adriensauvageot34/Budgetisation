...
const annualMonthFocus = await import("../src/features/global-v2/annual-month-focus.ts");
...
const backgroundRhythms = await transport({ resource: "analysis_global_background_rhythms", params: {} });
check(() => assert.equal(query.globalBackgroundRhythmsReadModelSchema.safeParse(backgroundRhythms.data).success, true));
check(() => assert.equal(backgroundRhythms.data.food.months.length, 12));
check(() => assert.equal(backgroundRhythms.data.carMobility.months.length, 12));
const decemberFood = backgroundRhythms.data.food.months.find(([month]) => month === "2025-12");
check(() => assert.deepEqual(decemberFood?.slice(1, 5), ["280.98", "113.90", "67.17", "462.05"]));
const marchCarDetail = await transport({ resource: "analysis_global_background_rhythm_month_detail", params: { domain: "CAR_MOBILITY", month: "2026-03" } });
check(() => assert.equal(query.globalBackgroundRhythmMonthDetailReadModelSchema.safeParse(marchCarDetail.data).success, true));
check(() => assert.equal(marchCarDetail.data.routineGroups.every(({ usageBand }) => usageBand === "AROUND_WORK" || usageBand === "OUTSIDE_WORK" || usageBand === "UNRESOLVED"), true));

const emptyMonthFocus = { rovingMonth: 0 };
const previewedMonthFocus = annualMonthFocus.annualMonthFocusReducer(emptyMonthFocus, { type: "PREVIEW", month: "2025-12" });
const selectedMonthFocus = annualMonthFocus.annualMonthFocusReducer(previewedMonthFocus, { type: "SELECT", month: "2025-12", index: 4 });
const hoveredOtherMonth = annualMonthFocus.annualMonthFocusReducer(selectedMonthFocus, { type: "PREVIEW", month: "2026-03" });
check(() => assert.equal(emptyMonthFocus.selectedMonth, undefined));
check(() => assert.equal(previewedMonthFocus.selectedMonth, undefined));
check(() => assert.equal(hoveredOtherMonth.selectedMonth, "2025-12"));
check(() => assert.equal(hoveredOtherMonth.previewMonth, "2026-03"));
check(() => assert.equal(annualMonthFocus.annualMonthFocusReducer(selectedMonthFocus, { type: "SELECT", month: "2025-12", index: 4 }).selectedMonth, undefined));
check(() => assert.equal(annualMonthFocus.annualMonthFocusReducer(selectedMonthFocus, { type: "SELECT", month: "2026-03", index: 7 }).selectedMonth, "2026-03"));
check(() => assert.equal(annualMonthFocus.nextRovingMonthIndex(11, "ArrowRight", 12), 0));
check(() => assert.equal(annualMonthFocus.nextRovingMonthIndex(0, "ArrowLeft", 12), 11));
check(() => assert.equal(annualMonthFocus.nextRovingMonthIndex(7, "Home", 12), 0));
check(() => assert.equal(annualMonthFocus.nextRovingMonthIndex(2, "End", 12), 11));
...
const backgroundSource = fs.readFileSync(path.join(root, "src/features/global-v2/background-rhythms.tsx"), "utf8");
const foodRiverSource = fs.readFileSync(path.join(root, "src/features/global-v2/food-rhythm-river.tsx"), "utf8");
const foodFocusSource = fs.readFileSync(path.join(root, "src/features/global-v2/food-month-focus.tsx"), "utf8");
const carRouteSource = fs.readFileSync(path.join(root, "src/features/global-v2/car-mobility-route.tsx"), "utf8");
const carFocusSource = fs.readFileSync(path.join(root, "src/features/global-v2/car-month-focus.tsx"), "utf8");
const monthInteractionSource = fs.readFileSync(path.join(root, "src/features/global-v2/annual-month-interaction-layer.tsx"), "utf8");
const annualMonthFocusSource = fs.readFileSync(path.join(root, "src/features/global-v2/annual-month-focus.ts"), "utf8");
const backgroundCssSource = fs.readFileSync(path.join(root, "src/features/global-v2/background-rhythms.module.css"), "utf8");
...
check(() => assert.doesNotMatch(routeSource, /GlobalV2ActivationPending|GLOBAL_V2_ROUTE_ACTIVE/u));
...
check(() => assert.match(navigationSource, /Synthèse[\s\S]*Nos dépenses[\s\S]*Catégories[\s\S]*Vie & dépenses[\s\S]*Profils/u));
...
check(() => assert.match(storyOrderSource, /"ECONOMIC", "CATEGORIES_NEEDS", "RHYTHM", "PERSONAS"/u));
...
const lifePrimarySource = [backgroundSource, foodRiverSource, foodFocusSource, carRouteSource, carFocusSource, monthInteractionSource].join("\n");
check(() => assert.match(backgroundSource, /analysis_global_background_rhythms/u));
...
check(() => assert.match(lifePrimarySource, /Nos rythmes de fond[\s\S]*Notre alimentation[\s\S]*Notre mobilité en voiture/u));
...
check(() => assert.match(foodFocusSource, /basketStructure\.status === "KNOWN"[\s\S]*Pas assez de passages renseignés/u));
check(() => assert.doesNotMatch(lifePrimarySource, /fuelExplainedShare|explainedAmount|variance|residual|paidVsEstimatedRatio/u));
...
check(() => assert.doesNotMatch(foodRiverSource, /useGlobalV2Resource|analysis_global_background_rhythm_month_detail/u));
check(() => assert.match(carRouteSource, /analysis_global_background_rhythm_month_detail[\s\S]*selected\?\.detailAvailable === true[\s\S]*"DIRECT"/u));
check(() => assert.match(foodFocusSource, /Pas assez de passages renseignés pour détailler la structure des paniers ce mois-ci\./u));
...
check(() => assert.doesNotMatch(lifePrimarySource, /dépense mensuelle M2|displayValue|percentile|person-activity|EUR\/person/u));
...
check(() => assert.match(backgroundCssSource, /\.monthInteractionLayer\s*\{[^}]*grid-template-columns:\s*repeat\(12/u));
check(() => assert.match(backgroundCssSource, /@media \(max-width: 600px\)[\s\S]*\.annualRiver\s*\{[^}]*height:\s*220px/u));
check(() => assert.match(backgroundCssSource, /@media \(prefers-reduced-motion: reduce\)[\s\S]*animation:\s*none/u));
check(() => assert.match(`${annualMonthFocusSource}\n${monthInteractionSource}`, /ArrowRight[\s\S]*ArrowLeft[\s\S]*Home[\s\S]*End[\s\S]*Enter[\s\S]*Escape/u));
check(() => assert.match(monthInteractionSource, /tabIndex=\{state\.rovingMonth === index \? 0 : -1\}/u));
check(() => assert.match(backgroundCssSource, /\.focusConnector\s*\{[^}]*left:\s*var\(--focus-connector-position/u));
check(() => assert.match(carFocusSource, /Voir le séjour dans notre Timeline/u));
check(() => assert.match(lifeTimelineSource, /function LegacyTimelineEventRow\(\{ event, focusRequest[\s\S]*data-global-entity-ref=\{event\.eventRef\}/u));
check(() => assert.doesNotMatch(lifePrimarySource, /role="dialog"|modal|drawer|bottom sheet|carousel|horizontal/u));
check(() => assert.doesNotMatch(carRouteSource, /Aveyron|Ski|Servian|marker|pin/iu));
check(() => assert.doesNotMatch(carRouteSource, />[^<]*(?:ratio|différence|% expliqué)[^<]*</iu));
