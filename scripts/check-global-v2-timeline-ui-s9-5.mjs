...
check(() => assert.match(css, /\.timelineMonth li > button, \.timelineMonth li > article\s*\{[^}]*grid-template-columns:\s*34px 28px minmax\(0, 3fr\) minmax\(0, 2fr\) 18px/u));
...
check(() => assert.match(css, /li\[data-has-expenses="false"\][^}]*grid-template-columns:\s*34px 28px minmax\(0, 1fr\)/u));
