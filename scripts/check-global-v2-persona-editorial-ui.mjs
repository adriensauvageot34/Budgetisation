...
for (const heading of ["Nos journées de travail", "Nos univers personnels", "Nos habitudes qui reviennent", "Notre vie sociale, chacun de son côté", "Notre Peugeot, son outil de travail quotidien", "Séries & divertissement", "Barbe & cheveux", "Ses produits fidèles", "Ses sorties de son côté", "Famille", "Une chanson pour son père"]) {
...
assert.match(view, /seule l’assurance est attribuée à Manon, pas l’entretien/);
assert.match(view, /Estimation d’usage, pas une facture de carburant/);
assert.match(view, /hairdresser\.personalAnnualCost === null \? null/);
...
assert.match(view, /Les achats ci-dessus ne sont pas répartis entre ces deux familles/);
