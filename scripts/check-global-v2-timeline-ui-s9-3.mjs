...
const panel = page.slice(page.indexOf("function GlobalLifeTimelinePanel"), page.indexOf("function ContextualSummaryPlaceholder"));
...
check(() => assert.match(panel, /timelineModuleHeader[\s\S]*timelineFocusControl[\s\S]*Élargir[\s\S]*Recentrer[\s\S]*<LifeTimeline/u));
...
check(() => assert.match(css, /\.timelineAccordion\s*\{[^}]*grid-template-rows:\s*0fr[^}]*210ms[^}]*opacity/u));
...
check(() => assert.match(css, /\.timelineChevron\s*\{[^}]*200ms[^}]*cubic-bezier[^}]*\}[\s\S]*@keyframes timelineChevronOpen/u));
check(() => assert.match(css, /li\[data-timeline-focused="true"\]\s*\{[^}]*background:[^}]*box-shadow:/u));
