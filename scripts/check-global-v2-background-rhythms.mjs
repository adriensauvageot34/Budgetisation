...
const detailFor = (month) => ({
  routineGroups: [{ routineGroupId: `routine:${month}`, pattern: "WORK_ONLY", title: "Trajets de travail", semanticFamily: "WORK", semanticTier: 1, occurrenceCount: 2, annualOccurrenceCount: 24, mobilityTripIds: [`trip:${month}:a`, `trip:${month}:b`], monthContribution: totals(2), fullTrips: totals(2) }],
  tripSummaries: [{ tripSummaryId: `summary:${month}`, mobilityTripId: `trip:${month}:c`, title: "Sortie", semanticFamily: "LEISURE", semanticTier: 2, usageBand: "OUTSIDE_WORK", multiDay: false, crossMonth: false, startDate: `${month}-10`, endDate: `${month}-10`, monthContribution: totals(), fullTrip: totals() }],
  contextOnly: [{ contextOnlyId: `context:${month}`, mobilityTripId: `trip:${month}:d`, title: "Anniversaire", semanticFamily: "FAMILY", semanticTier: 1, relationType: "PRIMARY_CONTEXT", targetKind: "MOMENT", targetRef: `moment:${month}` }],
